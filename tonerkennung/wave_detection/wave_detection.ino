...
void setup() {
  Serial.begin(9600);                  // starte Kommunikation mit dem Computer
  while (!Serial) {};                  // warte auf den Computer
  setup_frequency_recognition(A0);        // das Mikrofon wird an pin A0 angeschlossen
  Serial.print("Frequency of the wave is "); 
  Serial.print(get_frequency());       // diese Funktion gibt die Frequenz zurück
  Serial.println("Hz");                // die Einheit ist Hz
}

void loop() {
  long intensity = get_intensity_squared();     // Die Intensität quadriert
  Serial.print("intensity: ");                  // wird dem Computer mitgeteilt
  Serial.println(intensity);
  Serial.print("distance: ");
  Serial.println(1000. / pow(intensity, 0.25)); // die Distanz
  delay(100);
}
 ...
 ***************
 *
 * void setup_frequency_recognition(int microphone_pin)
 ...
 *
 * long get_intensity_squared()
 *    returns the loudness of the frequency.
 *
 * int get_average_sample()
 *    returns the average value of digitalRead(microphone_pin)
 *    It can be used to adjust the microphone.
 *    
 *************
 *  Program  *
 *************/
...
const int number_of_samples      = 300;  // number samples used to determine the intensity

  // see compute_squared_intensity()
  // 8 bit sinus_wave
  // 10 bit analogRead
...
const long overflow_preventing_intensity_divisor = 512; // <= 2048
...
long compute_squared_intensity() {

  long sin_value = sum_sinus / overflow_preventing_intensity_divisor;
  long cos_value = sum_cosinus / overflow_preventing_intensity_divisor;
  long squared_intensity = sin_value * sin_value + cos_value * cos_value;
  if (squared_intensity < 0 && Serial) {
    Serial.print(wave_detection_error);
    if (overflow_preventing_intensity_divisor >= 2048) {
      Serial.println("squared_intensity is negative but should be positive. overflow_preventing_intensity_divisor has the right value. What could be the reason?");
    } else {
      Serial.println("squared_intensity is negative but should be positive. Maybe overflow_preventing_intensity_divisor should be higher?");
    }
  }
  return squared_intensity;
}
...
long get_intensity_squared() {
  record_samples();
  convolve_wave();
  return compute_squared_intensity();
}
